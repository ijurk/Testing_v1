...
def get_fin(tickers):
    ...
    for ticker in tickers:
        ...
        if tickers.index(ticker) != 0 and tickers.index(ticker) % 200 == 0:
            print('We are on ticker number: ', tickers.index(ticker))
...
financial_dir_cy = get_fin(tickers_nan)


for ticker in tickers_nan:
    del financial_dir_cy_df[ticker]

financial_dir_cy_df.update(financial_dir_cy)

combined_financials_cy = pd.DataFrame(financial_dir_cy_df)

##CHECK UNTIL ALL DATA IS DOWNLOADED
...
financial_dir_cy_df.update(runnine_cy)
